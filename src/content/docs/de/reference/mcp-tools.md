...
[Konzepte](/de/concepts/data-model/).
...
| `scope` | nein | Ziel-Scope-Slug. Wird er weggelassen, entscheidet die **Standard-Schreib-Scope-Richtlinie** des Teams (`ask` / `project` / `global` — siehe [Konfiguration](/de/reference/configuration/)). |
...
- **Beschränkung auf den Benutzer.** Jeder Aufruf läuft als der authentifizierte
  ...
  einem anderen offenzulegen — siehe [Sicherheit & Datenschutz](/de/operations/security/).
