...
[Concepts](/en/concepts/data-model/).
...
| `scope` | no | Target scope slug. If omitted, the team's **default write-scope policy** decides (`ask` / `project` / `global` — see [Configuration](/en/reference/configuration/)). |
...
- **Scoping to the user.** Every call runs as the authenticated user. That is why
  ...
  [Security & privacy](/en/operations/security/).
